...
/**
 * Unit tests for UnmarkUnsellableController.
 */
final class UnmarkUnsellableControllerTest extends TestCaseBase {
	...
	public function testUnmarksOwnPlayerAndReturnsNull(): void {
		$i18n = $this->mockI18n(['myteam_remove_unsellable_player_success' => 'ok']);
		...
		$controller = new UnmarkUnsellableController($i18n, $ws, $db);
		$this->assertNull($controller->executeAction(['id' => 1]));
		$this->assertSame(0, $updated[0]['unsellable']);
	}
	...
	public function testThrowsWhenPlayerNotOwned(): void {
		...
		$ws = $this->mockWebsoccer($this->config());
		$ws->method('getUser')->willReturn($this->makeLoggedUser(1, 1));
		$db = $this->makeDb(['_spieler AS P' => [$this->playerRow(['team_id' => 9])]]);
