...
/**
 * Unit tests for UserResultsModel.
 */
final class UserResultsModelTest extends TestCaseBase {
	...
	public function testRenderViewReturnsFalseWhenNoUserId(): void {
		...
		$model = new UserResultsModel($this->mockDb(), $this->mockI18n(), $ws);
	...
	public function testGetTemplateParametersReturnsEmptyMatchesWhenNoData(): void {
		$ws = $this->ws(['db_prefix' => 'ws'], function ($name) { return ($name === 'userid') ? 5 : null; });
		$model = new UserResultsModel($this->mockDb(), $this->mockI18n(), $ws);
		$model->renderView();
