...
/**
 * Unit tests for LeagueDetailsModel.
 */
final class LeagueDetailsModelTest extends TestCaseBase {
	...
	public function testRenderViewAlwaysReturnsTrue(): void {
		...
		$model = new LeagueDetailsModel($this->mockDb(), $this->mockI18n(), $ws);
		$this->assertTrue($model->renderView());
	}

	public function testGetTemplateParametersReturnsNullLeagueWhenNoIdAndNoClub(): void {
		$user = $this->makeUser(['id' => null]);
		$ws = $this->websoccerWithUser($user, ['db_prefix' => 'ws']);
		$model = new LeagueDetailsModel($this->dbMock(), $this->mockI18n(), $ws);
		$params = $model->getTemplateParameters();
		$this->assertNull($params['league']);
		$this->assertSame([], $params['leagues']);
	}

	public function testGetTemplateParametersThrowsWhenLeagueIdProvidedButNotFound(): void {
		$user = $this->makeUser(['id' => 1]);
	...
	public function testGetTemplateParametersReturnsLeagueWhenFound(): void {
		$league = ['league_id' => 5, 'league_name' => 'Premier', 'league_short' => 'PRE', 'league_country' => 'EN'];
		$user = $this->makeUser(['id' => 1]);
		$db = $this->dbMock([], ['_liga AS L' => [$league]]);
		$ws = $this->websoccerWithUser($user, ['db_prefix' => 'ws'], ['id' => 5]);
		$model = new LeagueDetailsModel($db, $this->mockI18n(), $ws);
		$params = $model->getTemplateParameters();
		$this->assertSame($league, $params['league']);
		$this->assertSame([$league], $params['leagues']);
	}
}
