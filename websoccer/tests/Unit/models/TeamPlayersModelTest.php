...
/**
 * Unit tests for TeamPlayersModel.
 */
final class TeamPlayersModelTest extends TestCaseBase {
	...
	public function testRenderViewReturnsFalseWhenNoTeamId(): void {
		$ws = $this->ws(['db_prefix' => 'ws', 'players_aging' => 'birthday'], function() { return null; });
		$model = new TeamPlayersModel($this->mockDb(), $this->mockI18n(), $ws);
		$this->assertFalse($model->renderView());
	}

	public function testRenderViewReturnsTrueWhenTeamIdProvided(): void {
		$ws = $this->ws(['db_prefix' => 'ws', 'players_aging' => 'birthday'], function($name) { return ($name === 'teamid') ? 5 : null; });
		$model = new TeamPlayersModel($this->mockDb(), $this->mockI18n(), $ws);
		$this->assertTrue($model->renderView());
	}

	public function testGetTemplateParametersReturnsEmptyPlayersWhenTeamHasNone(): void {
		$ws = $this->ws(['db_prefix' => 'ws', 'players_aging' => 'birthday'], function($name) { return ($name === 'teamid') ? 5 : null; });
		$model = new TeamPlayersModel($this->mockDb(), $this->mockI18n(), $ws);
		$model->renderView();
