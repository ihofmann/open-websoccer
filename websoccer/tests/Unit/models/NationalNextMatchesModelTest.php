...
/**
 * Unit tests for NationalNextMatchesModel.
 */
final class NationalNextMatchesModelTest extends TestCaseBase {
	...
	public function testRenderViewReturnsTrueWhenEnabled(): void {
		...
		$model = new NationalNextMatchesModel($this->dbMock(), $this->mockI18n(), $ws);
	...
	public function testGetTemplateParametersThrowsWhenUserHasNoNationalTeam(): void {
		...
		$model = new NationalNextMatchesModel($this->dbMock([], 0), $this->mockI18n(['nationalteams_user_requires_team' => 'no team']), $ws);
		$this->expectException(Exception::class);
		$model->getTemplateParameters();
	}

	public function testGetTemplateParametersReturnsPaginatorAndEmptyMatchesWhenNoNextMatches(): void {
		$ws = $this->mockWebsoccer(['nationalteams_enabled' => TRUE, 'db_prefix' => 'ws']);
		$ws->method('getUser')->willReturn($this->makeUser(['id' => 1]));
		$db = $this->dbMock([['id' => 5]], 0);
		$model = new NationalNextMatchesModel($db, $this->mockI18n(), $ws);
