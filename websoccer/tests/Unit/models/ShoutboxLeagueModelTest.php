...
/**
 * Unit tests for ShoutboxLeagueModel.
 */
final class ShoutboxLeagueModelTest extends TestCaseBase {
	...
	public function testRenderViewReturnsFalseWhenNoId(): void {
		...
		$model = new ShoutboxLeagueModel($this->mockDb(), $this->mockI18n(), $ws);
	...
	public function testGetTemplateParametersReturnsEmptyMessagesAndHidesubmit(): void {
		$ws = $this->ws(['db_prefix' => 'ws'], function($name) { return ($name === 'id') ? 7 : null; });
		$model = new ShoutboxLeagueModel($this->mockDb(), $this->mockI18n(), $ws);
		$model->renderView();
		$params = $model->getTemplateParameters();
		$this->assertSame([], $params['messages']);
		$this->assertTrue($params['hidesubmit']);
	}
}
