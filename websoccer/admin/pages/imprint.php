...
$imprintPage = PageDataService::getByTypeAndLanguage(
	$website,
	$db,
...
if (!$show) {
  ...
  <p><?php echo $i18n->getMessage("imprint_introduction"); ?></p>
  
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="d-flex flex-wrap gap-2 align-items-center mb-3">
	<input type="hidden" name="site" value="<?php echo $site; ?>">
	
	<label for="lang"><?php echo $i18n->getMessage("imprint_label_language"); ?></label>
  ...
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="row">
	...
	$content = ($imprintPage) ? $imprintPage["content"] : '';
	
	$formFields["content"] = array("type" => "html", "value" => $content, "required" => "true");
